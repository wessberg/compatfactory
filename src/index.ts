			...
			...(missingCreateClassStaticBlockDeclaration
				? (() => {
					function createClassStaticBlockDeclaration (
						...
						const node = factory.createSemicolonClassElement() as unknown as Mutable<TS.ClassStaticBlockDeclaration>;
...
function createNodeFactory (typescript: typeof TS): TS.NodeFactory {
	...
	function createClassStaticBlockDeclaration (
		...
		// The closest we'll get at representing these nodes in a way that won't break the printer is with SemicolonClassElements
		const node = typescript.createSemicolonClassElement() as unknown as Mutable<TS.ClassStaticBlockDeclaration>;
